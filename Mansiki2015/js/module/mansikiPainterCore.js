define([
    ...
    var PainterCore = function(idbw,width,height,$area,$fileNameInput,$dlAncer,$fileForm,$clearButton){
    	...
    	mpf.buildBrushSelect($("#pensSelect"),this.mpcInst.setCurrentBrush.bind(this.mpcInst));
    	...
    	var $color2ndSetting = $('#color2ndSetting');
    	$color2ndSetting.minicolors(settings);
    	
    	psmPromise.done(function(){
    	    $("#MansikiBrushSelect_"+psm.ToolSelectionState.brush).click();
    	    var hex = self.mpcInst.currentBrush.color;
    	    alert("self.mpcInst.currentBrush:"+self.mpcInst.currentBrush.toSource());
    	    psm.changeColor(hex);
    	    $colorSetting.minicolors({'value':hex});
    	    var hex = self.mpcInst.currentBrush.color;
    	    psm.changeColor2nd(hex);
    	    $color2ndSetting.minicolors({'value':hex});
