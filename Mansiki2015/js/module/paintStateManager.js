define([
	...
	PaintStateManager.prototype={
		...
		getCurrentBrush:function(){
			...
			this.brushes[this.ToolSelectionState.brush]=mansikiPainterInitData.brushes.pen1;
			current = this.brushes[this.ToolSelectionState.brush];
		    }
		    current.color = this.ToolSelectionState.color;
		    return current;
		},
		save:function(){
		    ...
		    var d2= new $.Deferred();
                    ...
                    $.when(d1, d2).done(
                	function(){
                	    //alert("this.ToolSelectionState.brush:"+self.ToolSelectionState.brush);
                	    var brush = self.brushes[self.ToolSelectionState.brush];
                	    if(brush===undefined){
                		for(var index in mansikiPainterInitData.brushes){
                		    brush= mansikiPainterInitData.brushes[index];
                		    self.ToolSelectionState.brush = index;
                		    break;
                		}
                	    }else{
                		brush = mansikiPainterInitData.brushes[self.ToolSelectionState.brush].setData(brush);
                	    }
                	    self.mpc.setCurrentBrush(brush);
            		    d.resolve();
                	}
                    );
                    return d;
